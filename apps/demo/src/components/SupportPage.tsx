...
const faqItems = [
  {
    question: "本文は保存されますか？",
    answer:
      "保存しません。貼り付け本文、送信本文、添付前に読み取ったテキスト本文、検出結果、placeholderMap、送信履歴は永続保存しません。保存するのは拡張機能の設定と、検証済みの署名付きリモートルールキャッシュだけです。リモートルールキャッシュには本文や検出結果を含めません。"
  },
  {
...
const knownLimitations = [
  ...
  "本文は永続保存されず、設定と検証済みルールキャッシュだけがブラウザ内に保存されます。",
  "保存済み設定とリモートルールキャッシュはOptions Pageの「設定を初期化」から削除できます。貼り付け本文や検出結果は保存していないため、削除対象には含まれません。",
  "ファイル添付前チェックはテキスト系ファイルのみが対象です。PDF、docx、xlsx、画像や画像OCRが必要なファイルは解析せず、安全判定済みとは扱いません。",
  "画像OCRは、外部OCR APIを使わない方針、ランニングコストゼロ、ブラウザ内処理、端末負荷、日本語精度の観点から、現時点では実装しない判断です。",
