...
export function MaskResult({
  ...
  return (
    <div className="flex h-full min-h-[430px] flex-col">
      ...
      <pre
        className="flex-1 whitespace-pre-wrap break-words rounded-card border border-ink/10 bg-[#15191c] p-4 text-sm leading-6 text-[#e9eee9] inset-shadow-sm"
        aria-label="安全化後テキスト"
      >
        {maskedText || "検出を実行すると、ここに安全化後のテキストが表示されます。"}
      </pre>
      {copyMessage && <p className="mt-3 text-sm font-black text-leaf" role="status">{copyMessage}</p>}
    </div>
  );
}
