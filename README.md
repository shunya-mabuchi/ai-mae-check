...
0.1.0の公開ZIPに対応するルール配信用 `privateJwk` は手元に残っていないため、署名付きルール配信の本番有効化は0.1.1で鍵ペアを再発行して対応します。計画は [docs/release-0.1.1-rule-delivery-plan.md](docs/release-0.1.1-rule-delivery-plan.md)、運用メモは [docs/rule-delivery-operations.md](docs/rule-delivery-operations.md) にまとめています。
