...
**公開ステータス:** Chrome Web Storeへ0.1.0を送信済みで、現在は審査待ちです。審査中はZIP差し替えや新バージョン提出を行わず、公開後にLPの主CTAをChrome Web Store追加リンクへ差し替える予定です。
...
Chrome Web Store審査中は、次の順番で確認できるようにしています。

1. 紹介LPで、AIまえチェックがChrome拡張を本体とする送信前DLPレイヤーであることを確認する
2. LP内の「審査中の試し方」から、GitHub、ローカル読み込み、対象サイトQAの流れを確認する
...
ミニデモは導入前の補助体験です。ポートフォリオとしては体験価値を説明する入口にしつつ、READMEとLPではChrome拡張がプロダクト本体であることを先に示します。Chrome Web Store公開後は、LPの主CTAを「Chrome Web Storeで追加」へ差し替えます。
...
Chrome Web Store提出・審査中の説明文、権限理由、プライバシー方針、画像素材、手動チェックリストは [docs/chrome-web-store-release.md](docs/chrome-web-store-release.md) にまとめています。審査画面へ貼る文章は [docs/chrome-web-store-submission-copy.md](docs/chrome-web-store-submission-copy.md)、ストア画像の最終アップロード順は [docs/chrome-web-store-assets.json](docs/chrome-web-store-assets.json) で管理しています。

ルール配信Workerへ送るのは `GET /api/rules/latest` だけで、貼り付け本文・送信本文・検出結果・placeholderMap は送信しません。外部LLM APIも使わず、WebLLMの推論もブラウザ内で完結します。

0.1.0の審査中ZIPに対応するルール配信用 `privateJwk` は手元に残っていないため、署名付きルール配信の本番有効化は0.1.1で鍵ペアを再発行して対応します。計画は [docs/release-0.1.1-rule-delivery-plan.md](docs/release-0.1.1-rule-delivery-plan.md)、運用メモは [docs/rule-delivery-operations.md](docs/rule-delivery-operations.md) にまとめています。

プライバシー方針の本文は [docs/privacy-policy.md](docs/privacy-policy.md) にあります。

ChatGPT / Claude / Gemini上での実サイトQA手順は [docs/extension-site-qa.md](docs/extension-site-qa.md) にまとめています。WebLLMの実機確認観点は [docs/webllm-real-device-check.md](docs/webllm-real-device-check.md) に分けています。
...
LP上では、Chrome Web Store審査中の状態を表示し、GitHubからのローカル読み込み手順へ誘導します。Chrome Web Store公開後は、この導線をストア追加ボタンに差し替え、GitHubとミニデモは補助リンクとして残す想定です。
...
- 0.1.0では署名付きルール配信の本番有効化は見送り、同梱ルールへフォールバックする状態です。0.1.1で鍵ペアを再発行して有効化する予定です
...
- Perplexity adapter
- 秘密情報保護ルールの強化
- 0.1.1でのルール配信署名鍵再発行とCloudflare Secret反映
- PDF / docx / xlsx / 画像OCRなど、非テキストファイルの安全な検査方法
- Chrome Web Store公開後のLP CTA差し替えとGitHub Release v0.1.0公開
