...
ローカル確認時にブラウザや実行環境がモデル配信元へのアクセスをブロックしている場合、AI文脈チェックは実行できません。その場合も、ルールベースの検出とマスキングは引き続き利用できます。
...
- WebLLMのデフォルトモデルは`Llama-3.2-1B-Instruct-q4f32_1-MLC`です。`q4f16_1`を指定している古い設定は、Intel内蔵GPUなどでの互換性を優先して`q4f32_1`へ寄せます。
- デフォルトモデルがWebLLMのprebuilt model listにない場合は、低VRAM向けの`SmolLM2-360M-Instruct-q4f32_1-MLC`など、利用可能な軽量モデルへフォールバックします。
