...
4. 文脈チェック候補では、ユーザーがモーダル内の `AI文脈チェックも実行` を押した場合だけWebLLMを起動する
5. ユーザーが送信ボタンやEnterで送信しようとする
6. リスクがなければ通常どおり送信する
7. 注意情報がある場合は送信前に確認モーダルを表示する
8. ユーザーはカテゴリごとに安全化対象を確認する
9. `Mask` / `Generalize` / `Minimize` のいずれかで安全化する
10. 安全化後のテキストを再スキャンし、Secret Guardが残っていない場合に送信する
...
- 敬称つき人名、候補者名、担当者名、案件名が安全化後に残る場合の追加マスク
...
AIまえチェックでは、国産・日本語向けの推奨ソースモデルとして `sbintuitions/sarashina2.2-1b-instruct-v0.1` を採用候補にします。SB IntuitionsのSarashina2.2 1B instructはMIT Licenseで公開されており、日本語の指示追従モデルとして、顧客名・人名・案件名・社内文脈の候補確認に向いていると判断しています。

ただし、WebLLMは通常のHugging Faceモデルをそのまま読み込む仕組みではありません。現時点の `@mlc-ai/web-llm@0.2.84` のprebuilt model listにはSarashina2.2 1B instructのMLC変換済みモデルが含まれていないため、実行時にはWebLLMで利用可能な日本語向け互換モデル `gemma-2-2b-jpn-it-q4f32_1-MLC` へフォールバックします。この互換モデルも利用できない環境では、従来どおり `Llama-3.2-1B-Instruct-q4f32_1-MLC` などの軽量互換モデルへフォールバックします。

実際にSarashina2.2 1B instructをブラウザ内WebLLMで動かすには、MLC-LLM形式への変換、WebGPU向けmodel libraryの用意、配信元とライセンス表記、複数端末でのロード検証が必要です。初期版では、プロダクト方針としてSarashinaを国産推奨の実験候補に据えつつ、初期実行モデルは動作安定性を優先して `Llama-3.2-1B-Instruct-q4f32_1-MLC` にしています。

日本語の人名・会社名・案件名の抽出精度はモデルだけに依存させません。WebLLMが候補を返さない場合でも、入力文に実在する敬称つき人名や `Project ...` 形式の案件名は、ローカル補助候補として確認UIに出します。これにより、軽量モデルのままでも見落としを減らし、SarashinaのMLC変換が整った後もルールベース検出と補助候補を組み合わせて使える設計にしています。

候補として次もOptions Pageから選べるようにしています。

- 国産推奨モデル（実験）: `sbintuitions/sarashina2.2-1b-instruct-v0.1`
- 日本語互換モデル: `gemma-2-2b-jpn-it-q4f32_1-MLC`
- 互換性優先モデル・初期値: `Llama-3.2-1B-Instruct-q4f32_1-MLC`
- 低VRAMモデル: `SmolLM2-360M-Instruct-q4f32_1-MLC`
- 手動入力モデルID

`Llama-3.2-1B-Instruct-q4f16_1-MLC` はより軽い候補ですが、端末やブラウザのWebGPU機能によってはf16系の実行条件が合わない可能性があるため、初期値にはしていません。
...
- Sarashina2.2 1B instructは国産推奨ソースモデルとして扱いますが、現時点ではWebLLM用のMLC変換済み成果物を同梱していません。選択時はWebLLM互換モデルへフォールバックします
- `No available WebGPU adapters` が出る場合、ChromeがWebGPUの実行先アダプタを返していません。この状態はモデル変更では解消しないため、`chrome://gpu` のDawn InfoでD3D12 backendが利用可能か、WebGPU StatusがBlocklistedではないかを確認してください
- WebGPU推論中に `GPUBuffer.mapAsync` などの実行時エラーが出る場合があります。その場合は設定画面で互換性優先モデルまたは低VRAMモデルに切り替えてください
...
- 安全化後テキストに入力由来の敬称つき人名や `Project ...` 形式の案件名が残る場合は、メモリ上の後処理で追加マスクします。ただし日本語人名を完全に網羅するものではありません
...
- Sarashina2.2 1B instructのMLC変換、配信、WebGPU実機検証
- Chrome Web Store公開用のアイコンとスクリーンショット整備
- GitHub ActionsによるCI

## スクリーンショット掲載予定

Chrome Web Storeとポートフォリオ掲載用の初期画像案を `docs/assets/store/` に用意しています。画像内のデータはすべて実在しないダミーです。

![LP兼デモサイト](docs/assets/store/screenshot-01-lp.png)

![デモUI](docs/assets/store/screenshot-02-demo.png)

![送信前確認モーダル](docs/assets/store/screenshot-03-extension-modal.png)
