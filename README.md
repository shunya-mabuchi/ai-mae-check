...
初期設定では `Llama-3.2-1B-Instruct-q4f32_1-MLC` を使います。`@mlc-ai/web-llm@0.2.84` のprebuilt model listで利用可能な軽量モデルで、1Bクラスのため初回モデル取得量と推論負荷を抑えつつ、`q4f32` によりf16非対応環境も考慮した選択です。

結論として、初期モデルは「Chrome拡張で多くの端末に試してもらう」目的では妥当です。一方で、日本語の人名・会社名・案件名の抽出精度はモデルだけに依存させません。WebLLMが候補を返さない場合でも、入力文に実在する敬称つき人名や `Project ...` 形式の案件名は、ローカル補助候補として確認UIに出します。これにより、軽量モデルのままでも見落としを減らし、必要に応じて精度重視モデルへ切り替えられる設計にしています。

候補として次もOptions Pageから選べるようにしています。

- 低VRAMモデル: `SmolLM2-360M-Instruct-q4f32_1-MLC`
- 精度重視モデル: `Qwen2.5-1.5B-Instruct-q4f32_1-MLC`
- 手動入力モデルID

`Llama-3.2-1B-Instruct-q4f16_1-MLC` はより軽い候補ですが、端末やブラウザのWebGPU機能によってはf16系の実行条件が合わない可能性があるため、初期値にはしていません。日本語の固有名詞や文脈判断をより重視する場合は、端末負荷を確認しながらQwen系モデルを試す想定です。
