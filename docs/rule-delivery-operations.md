...
- ルールバンドルには明示的な `expiresAt` を入れ、署名検証後に期限切れバンドルを採用しない
- 緊急停止が必要な場合は、署名済みpayloadの `deliveryStatus: "paused"` でリモートルールだけを停止する
- 署名検証に失敗したルールはキャッシュしない
- ルールカタログ拡充は、fixture、ReDoS観点、誤検出観点、ストア説明への影響を確認してから行う

この判断により、0.1.1では「配信で検出対象を増やせる」こと、「期限切れルールを採用しない」こと、「壊れた配信時に同梱ルールへ戻れる」ことを優先します。複数バージョン保持は、実運用で必要になった段階で追加します。
...
検出ルール作成ガイド、同梱ルールカタログ、命名規則、誤検出を避けるレビュー観点は [detection-rule-authoring.md](./detection-rule-authoring.md) を参照してください。fixture追加とPRチェックリストは [dlp-rule-quality-process.md](./dlp-rule-quality-process.md) にまとめています。
...
- 署名検証に成功しても、payloadの `generatedAt` が未来すぎる場合や `expiresAt` が期限切れの場合は採用しない
- 署名検証に成功しても、payloadの `deliveryStatus` が `paused` の場合はリモートルールを採用せず、キャッシュも使わず同梱ルールへ戻る
- 同梱ルールベース検出は継続する
- 通信失敗またはHTTPエラーの場合は、最後に検証済みの署名付きリモートルールキャッシュを短時間だけ利用できる
- キャッシュ採用時も、署名、`keyId`、schema、`version`、`generatedAt`、`expiresAt`、有効期限を再確認する
...
- キャッシュTTLは30分のまま維持する。短すぎると一時障害に弱く、長すぎると壊れた配信の影響が残りやすいため、0.1.xでは中間値として扱う
- 複数バージョンのキャッシュ保持は採用しない。保存領域と検証分岐を増やすより、壊れたルールは新しい署名済みversionでロールバックする
- 配信停止フラグは採用する。ただし署名payload内の `deliveryStatus: "paused"` だけを信頼し、署名なしの停止指示やHTTPエラーは停止指示として扱わない

## 障害時の見え方

- Workerが落ちている場合: 期限内かつ `expiresAt` 内の検証済みキャッシュがあれば採用し、なければ同梱ルールへフォールバックする
- Secret未設定の場合: APIは署名設定なしのエラーを返す
- 署名不一致の場合: 拡張側はリモートルールを採用せず、キャッシュも使わず同梱ルールへ戻る
- `expiresAt` 期限切れの場合: 拡張側はリモートルールを採用せず、キャッシュも使わず同梱ルールへ戻る
- `deliveryStatus: "paused"` の場合: 拡張側は署名検証後にリモートルールを採用せず、キャッシュも使わず同梱ルールへ戻る
