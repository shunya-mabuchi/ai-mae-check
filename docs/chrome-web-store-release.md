...
- サイズ: 4,636,822 bytes
- SHA-256: `1FBE5992F1DDB93FD63F260B04944CB7C14B7DD0AEB81D3AF4933826AE9E8072`
...
拡張機能のロジックとして外部から任意のコードを取得して実行しません。WebLLMの初回利用時にはローカル推論用のモデルファイルを取得する場合がありますが、これは推論用モデルデータであり、ユーザー本文を外部LLM APIへ送信するものではありません。ルール配信を有効にしている場合も、取得するのはGitHub Pages上の `GET /rules/latest.json` で配信する署名付き静的ルールJSONのみで、リクエスト本文は使用しません。
...
2026-06-27時点で、残っている自動対応可能Issueを反映した0.1.1 ZIPを再生成し、Chrome Web Storeへ審査送信しました。2026-07-03に公開を確認した対象は次です。
