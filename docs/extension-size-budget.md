...
AIまえチェックはローカルAI bridgeを含むため、通常の小さなChrome拡張よりJavaScript bundleが大きくなります。Chrome Web Storeの上限だけを基準にすると肥大化に気づきにくいため、公開前QAとして内部予算を置きます。
...
| 展開後の `chrome-mv3` | 35MB以下 | ローカルAI workerやchunksを含めた実体サイズの肥大化を見るためです。 |
| 単一JavaScriptファイル | 8MB以下 | ローカルAI由来の大きなchunkを許容しつつ、意図しない依存混入を検知します。 |
...
- ローカルAI関連chunkが増えた場合は、重複bundleやworker読み込み経路を確認する
