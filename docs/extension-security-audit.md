...
- `<all_urls>` を無条件に要求しない
- 対象サイトは ChatGPT / Claude / Gemini / Perplexity に限定する
- 権限は単一用途に必要な範囲だけにする
- 外部LLM APIを使わない
...
- [ ] `host_permissions` が `https://chatgpt.com/*`, `https://chat.openai.com/*`, `https://claude.ai/*`, `https://gemini.google.com/*`, `https://www.perplexity.ai/*`, `https://perplexity.ai/*` に限定されている
- [ ] `<all_urls>` が入っていない
