...
- ChatGPT / Claude / Gemini の入力欄を見つける。
...
- #290 で `ContextHintResult` と `ContextBuilder` を追加し、WebLLM入力を全文前提から短いwindowへ寄せる。
- #291 でWebLLM準備済み時だけ自動実行する。
- #293 でWebLLM実行状態を `LocalLlmRuntimeService` としてUIから分離する。
...
| レイヤー | 現在の主な場所 | 今後の整理 |
| --- | --- | --- |
| Site Adapter Layer | `apps/extension/src/content/adapters/*` | Perplexity追加時もadapter単位で閉じる |
| Fast DLP Engine | `packages/core/src/detect.ts`, `remoteRules.ts` | #294でカテゴリ別detectorsへ段階分割 |
| Policy Decision | `packages/core/src/policy.ts`, `pasteGuard.ts`, `contentReview.ts` | #295で判定責務をさらに集約 |
| Context Risk Engine | `packages/llm/src/*`, `reviewLlmRunner.ts`, LLM bridge | #290, #291, #293で入力短縮と実行状態分離 |
| Sanitizer | `packages/core/src/transform.ts`, `mask.ts` | 安全化方式の追加時もcoreへ集約 |
| UX Layer | 拡張モーダル、Options Page、LP/デモ | #334で拡張モーダルをプロダクトUIへ再設計 |
...
| Issue | 位置づけ |
| --- | --- |
| #290 | ContextHintResultとContextBuilderでWebLLM入力を短縮する |
| #291 | WebLLM準備済み時だけAI文脈チェックを自動実行する |
| #292 | DLP評価fixtureとevalコマンドを追加する |
| #293 | LocalLlmRuntimeServiceでWebLLM実行状態をUIから分離する |
| #294 | 検出ルールをカテゴリ別detectorsへ段階的に分割する |
| #295 | PolicyDecisionを独立した判定エンジンとして整理する |
| #334 | 拡張モーダルをプロダクトUIとして再設計する |
