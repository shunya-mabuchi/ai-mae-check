...
- ルールベース検出を安全判定の主役にする。
- ローカルAIは、正規表現だけでは拾いにくい文脈上の注意候補を出す補助機能に限定する。
- ローカルAIに最終的な安全判定、送信可否、完全な安全宣言をさせない。
...
- モーダル表示や送信前の一次判断は、ローカルAIのモデルロードや推論完了を待たない。
...
- ローカルAI候補を補助情報として扱い、主判定をルールベースから外さない。

やらないこと:

- 検出ルールの実行。
- UIのDOM描画。
- ローカルAIの固定文脈分類とNER候補変換。
...
- ローカルAI候補は補助情報であり、LLM結果だけで完全遮断を決めない方針を維持する
...
ローカルAIとローカル補助候補により、ルールだけでは拾いにくい文脈上の注意候補を出す層です。
...
- ローカルAIへ渡す本文は `ContextBuilder` で短いwindowに絞り、全文を前提にしない。
- Chrome拡張MV3では常時待機できないため、ローカルAIがbridge内で準備済みの場合だけ自動実行し、未準備時は手動ボタンを残す。
- CPU/WASM初期化失敗、モデル取得失敗、候補変換失敗でもFast DLP Engineを止めない。
- ローカルAIの結果を「候補」として返す。
...
- ローカルAIの状態を「補助候補」として表示する。
- CPU/WASM初期化失敗やモデル取得失敗時に、日本語で復旧可能な説明を出す。
...
6. ローカルAIは必要に応じて追加候補を提示するが、送信可否の主判定にはしない。
...
| #386 | 完了 | Chrome Web Store 0.1.1を再提出し公開後導線を更新する |
| #388 | 実装済み | Chrome拡張E2Eハーネスを実装してローカル模擬composerでpaste/submitを検証する |
| #390 | 実装済み | 署名付きルール配信の複数鍵ローテーションに対応する |
| #391 | 実装済み | 検証済みリモートルールの短期キャッシュとTTLを実装する |
| #392 | 実装済み | 安全化の内部変換モードをredact / placeholder / generalize単位で整理する |
| #393 | 実装済み | 拡張モーダルのCSS定義を共通化して重複を減らす |
| #401 | 実装済み | READMEとロードマップの完了済みタスク表記を最新化する |
| #402 | 実装済み | Chrome拡張E2Eハーネスをキーボード送信とLexical / ProseMirror風DOMへ拡張する |
| #403 | 文書化済み | PDF / docx / xlsxは0.1.xでは解析対象外とし、検討条件を整理する |
| #404 | 文書化済み | 署名付きルール配信のTTL、鍵、ロールバック、ルールカタログ運用を整理する |
| #405 | 実装済み | criticalは独立blockではなく安全化必須の最重度として扱う方針をテストで固定する |
| #406 | 文書化済み | 対象サイト独自のドラッグ&ドロップ添付とクリップボード経由ファイル添付は0.1.xでは動作保証外と明記する |
