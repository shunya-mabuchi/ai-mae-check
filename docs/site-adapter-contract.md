...
0.1.xで有効なSiteAdapterとして扱います。2026-06-27の実サイト手動QAでは、paste確認、送信前確認、安全化後の入力反映が通過しています。Perplexityは検索モード、添付、音声入力などの近接ボタンが入力欄周辺に出る可能性があるため、adapterは送信ボタン候補を広げすぎず、実際の送信操作だけを対象にします。0.2系では添付経路を別判断とし、ファイル添付のdrop/pasteを安全に置換またはキャンセルできるかをSiteAdapter単位で確認します。対応範囲を広げる場合も `<all_urls>` は要求しません。
