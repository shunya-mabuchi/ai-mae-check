...
また、ブラウザ内AIによる文脈チェックにより、正規表現だけでは拾いにくい人名、組織名、場所、施設、顧客名、案件名、契約・採用・給与・法務などの候補を確認できます。小型日本語NERとRuri-v3-30mをTransformers.js + ONNX Runtime WebのCPU / WebAssemblyで実行します。AI文脈チェックは補助的な候補提示であり、完全な安全判定を行うものではありません。

貼り付け本文、送信本文、添付前に読み取った対応ファイル本文は永続保存しません。ユーザー設定と、検証済みの署名付きリモートルールキャッシュだけをブラウザ内の chrome.storage.local に保存します。外部LLM APIや開発者のサーバーへ本文を送信しません。ローカルAIの初回利用時には、ローカル推論用のモデルファイルを取得する場合があります。
...
・安全化候補の表示
・高リスク情報のそのまま送信抑止
・小型日本語NERとRuri-v3-30mによるブラウザ内AI文脈チェック
