...
また、ブラウザ内AIによる文脈チェックにより、正規表現だけでは拾いにくい顧客名候補、人名候補、会社名候補、案件名候補、契約・採用・給与・法務などの文脈上注意したい候補を確認できます。WebGPU上のWebLLMを利用できない場合は、CPU / WebAssemblyの小型分類モデルへ切り替えます。AI文脈チェックは補助的な候補提示であり、完全な安全判定を行うものではありません。

貼り付け本文、送信本文、添付前に読み取った対応ファイル本文は永続保存しません。ユーザー設定と、検証済みの署名付きリモートルールキャッシュだけをブラウザ内の chrome.storage.local に保存します。外部LLM APIや開発者のサーバーへ本文を送信しません。WebLLMまたはCPU文脈チェックの初回利用時には、ローカル推論用のモデルファイルを取得する場合があります。
...
・安全化候補の表示
・高リスク情報のそのまま送信抑止
・WebLLMとCPUフォールバックによるブラウザ内AI文脈チェック
