...
| `gpu_runtime_error` | `GPUBuffer.mapAsync` / buffer unmapped / `DXGI_ERROR_DEVICE_HUNG` / `Device was lost` / `Object has already been disposed` | 標準で失敗した場合は同じGemmaモデルを低負荷で1回だけ再実行し、失敗が続く場合はAI文脈チェック未完了を明示してルール検出とローカル補助候補を維持する | 自動復旧後の結果、低負荷プロファイル、再読み込み、Chrome完全再起動、GPUドライバ状態、WebGPU負荷を確認する |
