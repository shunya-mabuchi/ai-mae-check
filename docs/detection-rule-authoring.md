...
AIまえチェックの検出ルールは、メールアドレスやAPIキー風文字列のように確定的に拾いやすい情報を、ブラウザ内で素早く検出するためのものです。ローカルAIは文脈上の注意候補を補助する役割であり、高リスク情報や秘密情報保護対象の一次判定はルールベース検出を優先します。

この文書では、新しい検出ルールを追加するときの命名、危険度、placeholder、テスト、署名付き配信前レビューをまとめます。fixture追加、false positive確認、PRチェックリストは [dlp-rule-quality-process.md](./dlp-rule-quality-process.md) も参照してください。
...
- バンドル制御: `deliveryStatus` が `paused` の署名済みbundleは採用せず、同梱ルールへフォールバックする
...
| riskLevel | 判断基準 | 例 |
| --- | --- | --- |
| `critical` | 安全化なしでは送信させない最重度の重大情報。0.2系の再評価後も独立したblockではなく、`sanitize_required` の最重度として扱う。 | 本番秘密鍵、復旧不能な高権限token |
| `high` | そのまま送る前に安全化が必要な情報。秘密情報保護の対象になり得る。 | APIキー、秘密鍵、JWT、認証情報URL、Webhook URL、クレジットカード風番号、マイナンバー風文字列 |
...
配信bundle全体の運用状態を表します。通常は省略または `active` にします。

| 値 | 意味 |
| --- | --- |
| `active` | 署名検証、期限、schemaが有効であれば配信ルールを採用する |
| `paused` | 署名検証後にリモートルールを採用せず、同梱ルールへフォールバックする |

`deliveryStatus` は署名対象payloadに含めるため、改ざんされた停止指示は採用されません。壊れたルールを正しく署名してしまった場合や、緊急停止したい場合に使います。HTTPエラーや署名なしJSONを停止指示として扱わないのは、攻撃者や中間経路が勝手に配信停止状態を作れないようにするためです。
