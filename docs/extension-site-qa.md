...
0.1.xで有効な対象サイトとして扱います。2026-06-27のユーザー手動QAでは、paste確認、送信前確認、安全化後の入力反映が通過しています。検索モード、添付、音声入力などの近接ボタンを誤クリックしないことを継続確認します。0.2系では添付経路を別途調査し、ドラッグ&ドロップやクリップボード経由ファイル添付を安全に扱えない場合は対象外として明記します。対応を広げる場合も `<all_urls>` は要求しません。
