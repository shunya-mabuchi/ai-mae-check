...
2026-07-29時点で、GitHubの `main` ブランチ保護を有効化しています。

- PR経由の更新を必須にする
- 必須ステータスチェックを有効にする
- 必須チェックは `Typecheck, test, build, and release QA`
...
1人開発のためレビュー承認数は0にしていますが、PR自体は必須にしてCIを通してからmergeします。GitHub Pagesの本番デプロイは `main` へのmerge後に実行します。将来共同開発に移る場合は、承認数を1以上に上げます。
...
- GitHub Pages QAまたは本番デプロイが失敗した場合は、LP、`/privacy/`、`/support/`、`/api/rules/latest.json` の影響を確認する
