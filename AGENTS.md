...
- ユーザー本文を扱うバックエンドを作らない。追加ルールはGitHub Pages上の署名付き静的JSONとして配信し、ユーザー本文を受け取らない。
...
- ルールベース検出を最優先にする。
- APIキー、秘密鍵、JWT、`.env`、DATABASE_URL、トークン類、webhook URLなどは秘密情報保護の対象として扱う。
...
- 署名対象のルール定義は `rules`、GitHub Pages用の署名処理は `scripts` に置く。署名検証とリモートルールのDetectorRule化は `packages/core` に置く。
- WebLLMは文脈チェック専用に使う。依頼文生成や最終的な安全判定には使わない。
- メール、電話番号、APIキーなどの確定情報検出をWebLLMの主役にしない。
- 拡張機能と公開サイトのミニデモは同じcore検出エンジンを使う。
- WebLLMが失敗してもルールベース検出は利用できるようにする。
- 変換後テキストは再スキャンし、秘密情報保護の対象が残っていないか確認する。
...
- 秘密情報保護の対象が安全化なしで送信可能になっていないか確認する。
